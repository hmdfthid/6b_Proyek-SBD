...
export default () => {
  ...
  function hapus(id) {
      axios.delete(`http://localhost:8000/buku/${id}`)
      .then(function (response) {
          alert("Berhasil menghapus "+id);
      })
  }

  useEffect(() => {
      axios.get('http://localhost:8000/buku')
      .then(function (response) {
      ...
      <Card border="light" className="shadow-sm mb-4">
        <MaterialTable
            ...
            columns={[
              ...
              { title: 'Tahun Terbit', field: 'tahun_terbit', type: 'numeric' },
              { title: 'Pengarang', field: 'pengarang' },
            ...
            actions={[
              {
                icon: () => <Edit />,
                tooltip: 'Edit',
                onClick: (event, rowData) => edit(rowData.kode)
              },
              rowData => ({
                icon: () => <Delete />,
                tooltip: 'Delete',
                onClick: (event, rowData) => hapus(rowData.kode)
              }),
            {
